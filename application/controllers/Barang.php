...
class Barang extends CI_Controller
{
	...
	public function proses_tambah()
	{
		...
		$data = [
			'kode_ayam' => $this->input->post('kode_ayam'),
	...
	public function ubah($kode_ayam)
	{
		if ($this->session->login['role'] == 'kasir') {
		...
		$this->data['barang'] = $this->m_barang->lihat_id($kode_ayam);

		$this->load->view('barang/ubah', $this->data);
	}

	public function proses_ubah($kode_ayam)
	{
		if ($this->session->login['role'] == 'kasir') {
			$this->session->set_flashdata('error', 'Ubah data hanya untuk admin!');
			redirect('penjualan');
		...
		if ($this->m_barang->ubah($data, $kode_ayam)) {
	...
	public function hapus($kode_ayam)
	{
		...
		if ($this->m_barang->hapus($kode_ayam)) {
