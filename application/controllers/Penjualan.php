...
class Penjualan extends CI_Controller
{
	...
	public function tambah()
	{
		...
		$this->data['all_barang'] = $this->m_barang->lihat_jumlah();
	...
	{
		$jumlah_barang_dibeli = count($this->input->post('jenis_ayam_hidden'));
		...
		for ($i = 0; $i < $jumlah_barang_dibeli; $i++) {
			array_push($data_detail_penjualan, ['jenis_ayam' => $this->input->post('jenis_ayam_hidden')[$i]]);
		...
		if ($this->m_penjualan->tambah($data_penjualan) && $this->m_detail_penjualan->tambah($data_detail_penjualan)) {
			for ($i = 0; $i < $jumlah_barang_dibeli; $i++) {
				$this->m_barang->min_jumlah($data_detail_penjualan[$i]['jumlah_barang'], $data_detail_penjualan[$i]['jenis_ayam']) or die('gagal min stok');
			}
	...
	{
		$data = $this->m_barang->lihat_jenis_ayam($_POST['jenis_ayam']);
	...
	function konfirmasi($id)
	{
		...
		$this->session->set_flashdata('success', 'Invoice Penjualan <strong>Berhasil</strong> Dikonfirmasi!');
		redirect('penjualan');
	}
