							...
							<div class="card shadow">
								...
								<div class="card-body">
									<form action="<?= base_url('penjualan/proses_tambah') ?>" id="form-tambah" method="POST">
										...
										<div class="form-group col-3">
											<label for="jenis_ayam">Nama Barang</label>
											<select name="jenis_ayam" id="jenis_ayam" class="form-control">
												<option value="">Pilih Ayam</option>
												<?php foreach ($all_barang as $barang): ?>
													<option value="<?= $barang->jenis_ayam ?>"><?= $barang->jenis_ayam ?> <?= $barang->satuan ?></option>
												<?php endforeach ?>
											</select>
										</div>
											...
											<div class="form-group col-2">
												...
												<input type="text" name="kode_ayam" value="" readonly class="form-control">
											</div>
											<div class="form-group col-2">
												<label>Harga Ayam</label>
												<input type="text" name="harga_barang" class="form-control" value="" >
											</div>
											<div class="form-group col-2">
												<label>Jumlah</label>
											...
											<div class="form-group col-2">
												<label>Umur</label>
												<input type="text" name="satuan" class="form-control" value="" >
											</div>
		...
		$(document).ready(function() {
				...
				if ($(this).val() == '') reset()
					...
					$.ajax({
						...
						success: function(data) {
							$('input[name="kode_ayam"]').val(data.kode_ayam)
			...
			$(document).on('click', '#tambah', function(e) {
				...
				const data_keranjang = {
					jenis_ayam: $('select[name="jenis_ayam"]').val(),
				...
				if (parseInt($('input[name="max_hidden"]').val()) <= parseInt(data_keranjang.jumlah)) {
					...
					$.ajax({
						...
						data: data_keranjang,
						success: function(data) {
							if ($('select[name="jenis_ayam"]').val() == data_keranjang.jenis_ayam) $('option[value="' + data_keranjang.jenis_ayam + '"]').hide()
							reset()
			...
			$('button[type="submit"]').on('click', function() {
				$('input[name="kode_ayam"]').prop('disabled', true)
				$('select[name="jenis_ayam"]').prop('disabled', true)
